...
def NMF(X=None, k=None, options=None, bSuccess=None, U_=None, V_=None):
	...
	while tryNo < nRepeat:
		...
		while (maxErr > differror):
			...
			#print("V", V.flags["C_CONTIGUOUS"], V.flags["F_CONTIGUOUS"])
			#print("U", U.flags["C_CONTIGUOUS"], U.flags["F_CONTIGUOUS"])

			V = np.multiply(V, (X.T @ U) / np.maximum(V @ (U.T @ U), 1e-10))
			'''
			XU = np.matmul(np.transpose(X), U, order="F")
			UU = np.matmul(np.transpose(U), U, order="F")
			VUU = np.matmul(V, UU, order="F")
			V = np.multiply(V, np.divide(XU, np.maximum(VUU,1e-10, order="F"), order="F"), order="F")
			'''
			# ===================== update U ========================
			'''
			XV = X @ V
			...
			'''
			U = np.multiply(U, (X @ V) / np.maximum(U @ (V.T @ V), 1e-10))
			'''
			XV = np.matmul(X, V, order="F")
			VV = np.matmul(np.transpose(V), V, order="F")
			UVV = np.matmul(U, VV, order="F")
			U = np.multiply(U, np.divide(XV, np.maximum(UVV,1e-10, order="F"), order="F"), order="F")
			'''
			nIter = nIter + 1
